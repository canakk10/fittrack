...
const EXERCISES = [
  { id: 'yuruyus', label: 'Yürüyüş', unit: 'dk', kcalPerUnit: 4 },
  { id: 'kos', label: 'Koşu', unit: 'dk', kcalPerUnit: 9 },
  { id: 'bisiklet', label: 'Bisiklet', unit: 'dk', kcalPerUnit: 7 },
  { id: 'yuzme', label: 'Yüzme', unit: 'dk', kcalPerUnit: 8 },
  { id: 'fitness', label: 'Fitness', unit: 'dk', kcalPerUnit: 6 },
  { id: 'dans', label: 'Dans', unit: 'dk', kcalPerUnit: 5 },
  { id: 'diger', label: 'Diğer', unit: 'dk', kcalPerUnit: 5 },
];
...
export default function App() {
  ...
  const handlePhoto = (e) => {
    ...
    reader.onload = ev => {
      ...
      photos.push({ src: ev.target.result, date: todayStr(), label: fmtDate(todayStr()) });
      save({ ...st, photos });
    };
  ...
  const removePhoto = (i) => {
    const photos = (st.photos || []).filter((_, idx) => idx !== i);
    save({ ...st, photos });
  };
    ...
    return (
      <div style={{ position: 'relative' }}>
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: H, overflow: 'visible' }}>
          ...
          {yTicks.map(({ v, y }) => (
            <g key={v}>
              <line x1={PX} y1={y} x2={W - PX} y2={y} stroke="#eee" strokeWidth="0.5" strokeDasharray="3,3" />
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.app}>
        ...
        {tab === 'food' && <div style={S.screen}>
            ...
            {todayExercises.length > 0 && <div style={{ marginTop: 12 }}>
              {todayExercises.map((ex, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: i < todayExercises.length - 1 ? '0.5px solid #f0f0f0' : 'none' }}>
                  <div>
                    <div style={{ fontSize: 14 }}>{ex.type}</div>
                    <div style={{ fontSize: 11, color: '#888' }}>{ex.duration} dakika</div>
                  </div>
              ...
              <div style={{ paddingTop: 8, display: 'flex', justifyContent: 'space-between', borderTop: '0.5px solid #f0f0f0', marginTop: 4 }}>
          ...
          <div style={S.card}>
            <div style={{ marginBottom: 10 }}>
              <input type="text" value={foodInput} placeholder="örn. 2 yumurta, 1 dilim ekmek, ayran" onChange={e => setFoodInput(e.target.value)} style={S.inp} />
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
        ...
        {tab === 'progress' && <div style={S.screen}>
          ...
          <div style={S.card}>
            <div style={{ marginBottom: 12 }}>
              <label style={{ ...S.btnP, display: 'block', textAlign: 'center', cursor: 'pointer' }}>
                📷 Fotoğraf ekle
                <input type="file" accept="image/*" onChange={handlePhoto} style={{ display: 'none' }} />
              </label>
              <div style={{ fontSize: 12, color: '#888', textAlign: 'center', marginTop: 6 }}>Her fotoğrafa tarih otomatik eklenir</div>
            </div>
            {(st.photos || []).length === 0 && <div style={{ color: '#ccc', textAlign: 'center', padding: 16, fontSize: 13 }}>Henüz fotoğraf yok</div>}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {(st.photos || []).map((ph, i) => (
                <div key={i} style={{ position: 'relative' }}>
                  <img src={ph.src} style={{ width: '100%', aspectRatio: '3/4', objectFit: 'cover', borderRadius: 12 }} />
                  <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'linear-gradient(transparent, rgba(0,0,0,0.6))', borderRadius: '0 0 12px 12px', padding: '20px 8px 8px', color: 'white', fontSize: 11 }}>
                    {ph.label}
                  </div>
                  <div onClick={() => removePhoto(i)} style={{ position: 'absolute', top: 6, right: 6, background: 'rgba(0,0,0,0.5)', color: 'white', borderRadius: '50%', width: 24, height: 24, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 14 }}>×</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* SETTINGS */}
        {tab === 'settings' && <SettingsTab st={st} tdee={tdee} bmi={bmi} onSave={saveSettings} onReset={() => { if (confirm('Tüm veriler silinecek?')) { localStorage.removeItem('fittrack_v4'); window.location.reload(); } }} />}
