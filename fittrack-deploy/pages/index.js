...
// Persistent storage — IndexedDB for photos, localStorage for everything else
...
// Save state to both localStorage (fast) and IndexedDB (persistent)
async function persistState(st) {
  try {
    localStorage.setItem('fittrack_v4', JSON.stringify({ ...st, photos: undefined }));
  } catch {}
  try {
    await idbSet('state', 'main', st);
  } catch {}
}
...
async function loadState() {
  ...
  try {
    ...
    if (st && st.setupDone) return st;
  } catch {}
  // Fallback to localStorage
  try {
    const v = localStorage.getItem('fittrack_v4');
...
export default function App() {
  ...
  const save = async (newSt) => {
    setSt(newSt);
    await persistState(newSt);
  };

  const doSetup = async () => {
    const w = parseFloat(form.weight), h = parseFloat(form.height), t = parseFloat(form.target), age = parseInt(form.age);
    if (!w || !h || !t || !form.date || !age) { alert('Lütfen tüm alanları doldur!'); return; }
    const newSt = {
      setupDone: true, name: form.name || 'Dostum',
      startWeight: w, currentWeight: w, height: h,
      targetWeight: t, targetDate: form.date,
      kcalGoal: parseInt(form.kcal) || 1800,
      gender: form.gender, age,
  ...
  const confirmFood = async () => {
    ...
    fl[todayStr()] = [...fl[todayStr()], {
      name: foodInput, kcal: aiRes.kcal,
      detail: `${aiRes.protein}g protein · ${aiRes.karb}g karb · ${aiRes.yag}g yağ`
    }];
    await save({ ...st, foodLog: fl });
  ...
  const handlePhoto = async (n, e) => {
    const file = e.target.files[0]; if (!file) return;
    const reader = new FileReader();
    reader.onload = async ev => {
      const ph = { ...(st.photos || {}), [n]: ev.target.result };
      await save({ ...st, photos: ph });
    };
    reader.readAsDataURL(file);
  };

  const saveSettings = async (sf) => {
    const newSt = {
      ...st,
      name: sf.name || st.name,
      height: parseFloat(sf.height) || st.height,
      targetWeight: parseFloat(sf.target) || st.targetWeight,
      targetDate: sf.date || st.targetDate,
      kcalGoal: parseInt(sf.kcal) || st.kcalGoal,
      gender: sf.gender || st.gender,
      age: parseInt(sf.age) || st.age
    };
    await save(newSt);
    alert('Kaydedildi ✓');
  };

  if (!ready) return <div style={{ display:'flex', alignItems:'center', justifyContent:'center', minHeight:'100vh', fontSize:24 }}>💪</div>;
  ...
  if (!st || !st.setupDone) return (
    <div style={S.page}>
      ...
      <div style={{ maxWidth:390, margin:'0 auto', padding:'20px 16px 40px' }}>
        <div style={{ fontSize:40, marginBottom:8 }}>💪</div>
        <h1 style={{ fontSize:26, fontWeight:700, marginBottom:6 }}>Merhaba!</h1>
        <p style={{ fontSize:14, color:'#888', marginBottom:24, lineHeight:1.5 }}>Sana özel takip panelini kuralım.</p>

        <div style={{ marginBottom:12 }}>
          <label style={S.lbl}>Cinsiyet</label>
          <div style={{ display:'flex', gap:10 }}>
            {[['male','👨 Erkek'],['female','👩 Kadın']].map(([v,lbl]) => (
              <button key={v} onClick={() => setForm(f=>({...f,gender:v}))}
                style={{ flex:1, padding:'11px', borderRadius:10, border: form.gender===v ? '2px solid #1D9E75' : '1px solid #ddd', background: form.gender===v ? '#E1F5EE' : 'white', cursor:'pointer', fontSize:14, fontWeight: form.gender===v ? 600 : 400 }}>
                {lbl}
              </button>
            ))}
          </div>
        ...
        {[['Adın','name','text','Adını gir'],['Yaşın','age','number','30'],['Mevcut kilo (kg)','weight','number','94'],['Boy (cm)','height','number','183'],['Hedef kilo (kg)','target','number','77'],['Günlük kalori hedefi (kcal)','kcal','number','1800']].map(([lbl,k,type,ph]) => (
        ...
        <div style={{ marginBottom:12 }}>
          <label style={S.lbl}>Hedef tarih</label>
  ...
  const age = st.age || 30;
  const bmr = st.gender === 'female'
    ? (10 * st.currentWeight + 6.25 * st.height - 5 * age - 161)
    : (10 * st.currentWeight + 6.25 * st.height - 5 * age + 5);
  ...
  const ChartSVG = () => {
    if (sortedLogs.length < 2) return (
      <div style={{ textAlign:'center', padding:'28px 0', color:'#aaa', fontSize:13 }}>
        En az 2 gün kilo gir, grafik oluşsun 📈
      </div>
    );
    ...
    return (
      <div style={{ position:'relative' }}>
        ...
        {tooltip.visible && (
          <div style={{ position:'absolute', top: tooltip.y - 32, left:`calc(${tooltip.x}% - 60px)`, background:'#0F6E56', color:'white', borderRadius:8, padding:'4px 10px', fontSize:12, pointerEvents:'none', whiteSpace:'nowrap', zIndex:10 }}>
            {tooltip.text}
          </div>
        )}
      ...
      <div style={S.app}>
        ...
        {tab==='home' && <div style={S.screen}>
          ...
          <div style={S.statGrid}>
            ...
            <div style={S.statCard}>
              <div style={S.statLbl}>Günlük yakım</div>
              <div style={S.statVal}>{tdee}</div>
              <div style={{fontSize:12,color:'#888',marginTop:4}}>kcal/gün (TDEE)</div>
            </div>
          </div>
        ...
        {tab==='food' && <div style={S.screen}>
          ...
          <div style={S.card}>
            ...
            <div style={{marginBottom:12}}><input type="text" value={foodInput} placeholder="örn. 2 yumurta, 1 dilim ekmek, ayran" onChange={e=>setFoodInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&analyzeFood()} style={S.inp}/></div>
            <div style={{display:'flex',gap:8}}>
              <button onClick={analyzeFood} style={{...S.btnP,flex:1,width:'auto',opacity:aiLoad?0.7:1}} disabled={aiLoad}>{aiLoad?'Hesaplanıyor ⏳':'AI ile hesapla ✨'}</button>
            ...
            {todayFoods.map((f,i) => (
              <div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 0',borderBottom:i<todayFoods.length-1?'0.5px solid #f0f0f0':'none'}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:8}}>
                  ...
                  <span onClick={()=>removeFood(i)} style={{cursor:'pointer',color:'#ccc',fontSize:20,lineHeight:1}}>×</span>
                </div>
              </div>
            ))}
        ...
        {tab==='progress' && <div style={S.screen}>
          ...
          <div style={S.card}>
            {[...st.weightLog].sort((a,b)=>b.date.localeCompare(a.date)).slice(0,30).map((l,i,arr) => {
              const ws = arr.map(x=>x.weight), mn=Math.min(...ws), mx=Math.max(...ws), rng=mx-mn||1;
              ...
              return <div key={i} style={{display:'flex',alignItems:'center',gap:10,padding:'9px 0',borderBottom:i<arr.length-1?'0.5px solid #f5f5f5':'none'}}>
                <div style={{fontSize:12,color:isToday?'#1D9E75':'#888',width:95,flexShrink:0,fontWeight:isToday?600:400}}>{lbl}{isToday?' 🔵':''}</div>
                ...
                <div style={{fontSize:13,fontWeight:500,width:52,textAlign:'right'}}>{l.weight} kg</div>
              </div>;
            })}
          ...
          <div style={S.card}>
                ...
                <div key={n}>
                  <div onClick={()=>document.getElementById(`ph${n}`).click()}
                    ...
                    {st.photos && st.photos[n]
                      ...
                      : <><span style={{fontSize:28}}>📷</span><span>Ekle</span></>}
                  </div>
            ...
            <div style={{fontSize:11,color:'#bbb',textAlign:'center',marginTop:12}}>📱 Fotoğraflar cihazına kalıcı olarak kaydedilir</div>
          </div>
        </div>}

        {tab==='settings' && <SettingsTab st={st} tdee={tdee} bmr={Math.round(bmr)} bmi={bmi} onSave={saveSettings} onReset={async()=>{if(confirm('Tüm veriler silinecek?')){try{const db=await openDB();db.transaction(['state','photos'],'readwrite').objectStore('state').clear();}catch{}localStorage.removeItem('fittrack_v4');window.location.reload();}}}/>}
...
function SettingsTab({ st, tdee, bmr, bmi, onSave, onReset }) {
  const [sf, setSf] = useState({ name:st.name, height:st.height, target:st.targetWeight, date:st.targetDate, kcal:st.kcalGoal, gender:st.gender||'male', age:st.age||30 });
  return (
    <div style={S.screen}>
      ...
      <div style={S.card}>
        <div style={{marginBottom:12}}>
          ...
          <div style={{display:'flex',gap:10}}>
            {[['male','👨 Erkek'],['female','👩 Kadın']].map(([v,lbl]) => (
              <button key={v} onClick={()=>setSf(f=>({...f,gender:v}))}
                style={{flex:1,padding:11,borderRadius:10,border:sf.gender===v?'2px solid #1D9E75':'1px solid #ddd',background:sf.gender===v?'#E1F5EE':'white',cursor:'pointer',fontSize:14,fontWeight:sf.gender===v?600:400}}>
                {lbl}
              </button>
            ))}
          </div>
        </div>
        {[['Adın','name','text'],['Yaş','age','number'],['Boy (cm)','height','number'],['Hedef kilo (kg)','target','number'],['Hedef tarih','date','date'],['Günlük kalori hedefi','kcal','number']].map(([lbl,k,type]) => (
          <div key={k} style={{marginBottom:12}}>
            <label style={S.lbl}>{lbl}</label>
      ...
      <div style={S.card}>
        ...
        <div style={{fontSize:13,color:'#888',lineHeight:2.2}}>
          ...
          Bazal metabolizma (BMR): ~{bmr} kcal<br/>
