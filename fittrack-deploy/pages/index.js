...
export default function App() {
  ...
  const [form, setForm]     = useState({ name: '', weight: '94', height: '183', target: '77', date: '', kcal: '1800', gender: 'erkek', age: '30' });
  ...
  const doSetup = () => {
    ...
    save({ setupDone: true, name: form.name || 'Dostum', startWeight: w, currentWeight: w, height: h, targetWeight: t, targetDate: form.date, kcalGoal: parseInt(form.kcal) || 1800, gender: form.gender || 'erkek', age: parseInt(form.age) || 30, weightLog: [{ date: todayStr(), weight: w }], foodLog: {}, exerciseLog: {} });
    setTab('home');
  };
  ...
  const saveSettings = (sf) => {
    save({ ...st, name: sf.name || st.name, height: parseFloat(sf.height) || st.height, targetWeight: parseFloat(sf.target) || st.targetWeight, targetDate: sf.date || st.targetDate, kcalGoal: parseInt(sf.kcal) || st.kcalGoal, age: parseInt(sf.age) || st.age, gender: sf.gender || st.gender });
    alert('Kaydedildi ✓');
  };
  ...
  if (!st || !st.setupDone) return (
    <div style={S.page}>
      ...
      <div style={{ padding: '20px 16px', maxWidth: 390, margin: '0 auto' }}>
        ...
        <p style={{ fontSize: 14, color: '#888', marginBottom: 24, lineHeight: 1.5 }}>Sana özel takip panelini kurmak için birkaç bilgiye ihtiyacım var.</p>
        {[['Adın', 'name', 'text', 'Adını gir'], ['Mevcut kilo (kg)', 'weight', 'number', '94'], ['Boy (cm)', 'height', 'number', '183'], ['Yaş', 'age', 'number', '30'], ['Hedef kilo (kg)', 'target', 'number', '77'], ['Hedef tarih', 'date', 'date', ''], ['Günlük kalori hedefi (kcal)', 'kcal', 'number', '1800']].map(([lbl, k, type, ph]) => (
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            {[['erkek', '👨 Erkek'], ['kadin', '👩 Kadın']].map(([val, lbl]) => (
              <button key={val} onClick={() => setForm(p => ({ ...p, gender: val }))}
                style={{ flex: 1, padding: 10, borderRadius: 10, border: `2px solid ${form.gender === val ? '#1D9E75' : '#ddd'}`, background: form.gender === val ? '#E1F5EE' : '#fafafa', color: form.gender === val ? '#0F6E56' : '#888', fontWeight: form.gender === val ? 600 : 400, cursor: 'pointer', fontSize: 14 }}>
                {lbl}
              </button>
            ))}
          </div>
  ...
  const ChartSVG = () => {
    if (sortedLogs.length < 2) return <div style={{ textAlign: 'center', padding: '32px 0', color: '#aaa', fontSize: 13 }}>En az 2 gün kilo gir, grafik oluşsun 📈</div>;
    ...
    return (
      <div style={{ position: 'relative' }}>
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: H, overflow: 'visible' }}>
          ...
          {[minW, (minW+maxW)/2, maxW].map(v => { const y = yS(v); return <g key={v}><line x1={PX} y1={y} x2={W-PX} y2={y} stroke="#eee" strokeWidth="0.5" strokeDasharray="3,3"/><text x={PX-4} y={y+4} textAnchor="end" fontSize="9" fill="#bbb">{v.toFixed(0)}</text></g>; })}
      ...
      <div style={S.app}>
        ...
        {tab === 'food' && <div style={S.screen}>
          ...
          <div style={{ ...S.card, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px 16px 14px' }}>
            <svg width="130" height="130" viewBox="0 0 130 130">
              ...
              <text x="65" y="75" textAnchor="middle" fontSize="11" fill="#888">net kcal</text>
            </svg>
            <div style={{ fontSize: 13, color: '#888', marginTop: 8 }}>Hedef: {st.kcalGoal} kcal</div>
            <div style={{ display: 'flex', gap: 20, marginTop: 12 }}>
              <div style={{ textAlign: 'center' }}><div style={{ fontSize: 16, fontWeight: 600 }}>{todayKcal}</div><div style={{ fontSize: 11, color: '#888' }}>yenilen</div></div>
              <div style={{ textAlign: 'center' }}><div style={{ fontSize: 16, fontWeight: 600, color: '#1D9E75' }}>-{todayBurned}</div><div style={{ fontSize: 11, color: '#888' }}>yakılan</div></div>
              <div style={{ textAlign: 'center' }}><div style={{ fontSize: 16, fontWeight: 600 }}>{Math.max(0, kcalRem)}</div><div style={{ fontSize: 11, color: '#888' }}>kalan</div></div>
            </div>
          </div>
            ...
            {todayExercises.length > 0 && <div style={{ marginTop: 12 }}>
              {todayExercises.map((ex, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: i < todayExercises.length - 1 ? '0.5px solid #f0f0f0' : 'none' }}>
                  <div><div style={{ fontSize: 14 }}>{ex.type}</div><div style={{ fontSize: 11, color: '#888' }}>{ex.duration} dakika</div></div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 13, color: '#1D9E75', fontWeight: 500 }}>-{ex.burned} kcal</span>
                    <span onClick={() => removeExercise(i)} style={{ cursor: 'pointer', color: '#ccc', fontSize: 18 }}>×</span>
                  </div>
                </div>
              ))}
              <div style={{ paddingTop: 8, borderTop: '0.5px solid #f0f0f0', marginTop: 4, display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ fontSize: 13, color: '#888' }}>Toplam yakılan</span>
        ...
        {tab === 'progress' && <div style={S.screen}>
          ...
          <div style={S.card}>
            ...
            {photos.length === 0 && <div style={{ color: '#ccc', textAlign: 'center', padding: 16, fontSize: 13 }}>Henüz fotoğraf yok</div>}
...
function SettingsTab({ st, tdee, bmi, onSave, onReset }) {
  const [sf, setSf] = useState({ name: st.name, height: st.height, target: st.targetWeight, date: st.targetDate, kcal: st.kcalGoal, age: st.age || 30, gender: st.gender || 'erkek' });
  return (
    <div style={S.screen}>
      <div style={S.secTitle}>Profil & Ayarlar</div>
      <div style={S.card}>
        {[['Adın', 'name', 'text'], ['Boy (cm)', 'height', 'number'], ['Yaş', 'age', 'number'], ['Hedef kilo (kg)', 'target', 'number'], ['Hedef tarih', 'date', 'date'], ['Günlük kalori hedefi', 'kcal', 'number']].map(([lbl, k, type]) => (
          <div key={k} style={{ marginBottom: 12 }}>
            <label style={S.lbl}>{lbl}</label>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            {[['erkek', '👨 Erkek'], ['kadin', '👩 Kadın']].map(([val, lbl]) => (
              <button key={val} onClick={() => setSf(p => ({ ...p, gender: val }))}
                style={{ flex: 1, padding: 10, borderRadius: 10, border: `2px solid ${sf.gender === val ? '#1D9E75' : '#ddd'}`, background: sf.gender === val ? '#E1F5EE' : '#fafafa', color: sf.gender === val ? '#0F6E56' : '#888', fontWeight: sf.gender === val ? 600 : 400, cursor: 'pointer', fontSize: 14 }}>
                {lbl}
              </button>
            ))}
          </div>
